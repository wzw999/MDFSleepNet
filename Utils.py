...
def AddContext_MultiSub(x, y, Fold_Num, context, i):
    ...
    id_del = np.sort(id_del).astype(int)
...
def Instantiation_optim(name, learning_rate, **kwargs):
    if name == "adam":
        opt = keras.optimizers.Adam(learning_rate=learning_rate, **kwargs)
    elif name == "RMSprop":
        opt = keras.optimizers.RMSprop(learning_rate=learning_rate, **kwargs)
    elif name == "SGD":
        opt = keras.optimizers.SGD(learning_rate=learning_rate, **kwargs)
    else:
